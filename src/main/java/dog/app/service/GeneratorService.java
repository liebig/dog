...
public class GeneratorService implements Runnable {
    ...
    public void run() {
        ...
        for (DocumentToGenerate documentToGenerate : documentsToGenerate) {
            ...
            ArrayList<dog.app.domain.Image> images = documentToGenerate.getImages();
            ...
            try {
                ...
                if (outputType.equals(GeneratorService.PDF)) {
                    ...
                    toFile = new File(outputPath + File.separator + filename + ".pdf");

                    int fileVersion = 1;
                    // Check if file already exists, if true increment file
                    // version
                    while (toFile.exists()) {
                        logger.debug("File to write already exists, increment filename: " + toFile.getAbsolutePath());
                        toFile = new File(outputPath + File.separator + filename + "." + fileVersion + ".pdf");
                        fileVersion++;
                    }
                    ...
                    toFile = new File(outputPath + File.separator + filename + ".rtf");
                    ...
                    while (toFile.exists()) {
                        logger.debug("File to write already exists, increment filename: " + toFile.getAbsolutePath());
                        toFile = new File(outputPath + File.separator + filename + "." + fileVersion + ".rtf");
                        fileVersion++;
                    }

                    RtfWriter2.getInstance(document, new FileOutputStream(toFile));
                } else {
                    throw new IllegalArgumentException(
                            "Only the output type pdf or rtf is supported. Given type: " + outputType);
                }

                logger.debug("Writing to document: " + toFile.getAbsolutePath());
                Application.log("Writing to document: " + toFile.getAbsolutePath());
                ...
                String headerSuffix = Application.properties.getProperty("header-suffix", "");

                for (int i = 0; i < images.size(); i++) {

                    dog.app.domain.Image currentImage = images.get(i);

                    BufferedImage loadedImage = ImageIO.read(currentImage.getFile());

                    // Load maxResizingString from properties
                    String maxResizingString = Application.properties.getProperty("resizing-value", "NaN");
                    ...
                    if (maxResizingValue > 0) {
                        ...
                        BufferedImage currentBufferedImage = ImageIO.read(currentImage.getFile());
                        ...
                        // resizing value)
                        if (currentBufferedImage.getWidth() < currentBufferedImage.getHeight()) {
                            ...
                            if (currentBufferedImage.getHeight() > maxResizingValue) {
                                logger.debug("Resizing image " + currentImage.getFile().getName() + " to height "
                                        + maxResizingValue);
                                Application.log("Resizing image " + currentImage.getFile().getName());
                                loadedImage = Scalr.resize(loadedImage, Scalr.Method.QUALITY, Mode.FIT_TO_HEIGHT,
                                        maxResizingValue);
                            }
                        } else {
                            ...
                            if (currentBufferedImage.getWidth() > maxResizingValue) {
                                logger.debug("Resizing image " + currentImage.getFile().getName() + " to width "
                                        + maxResizingValue);
                                Application.log("Resizing image " + currentImage.getFile().getName());
                                loadedImage = Scalr.resize(loadedImage, Scalr.Method.QUALITY, Mode.FIT_TO_WIDTH,
                                        maxResizingValue);
                            }
                        }
                    }

                    String ext = FilenameUtils.getExtension(currentImage.getFile().getAbsolutePath()).toLowerCase();
                    ...
                    String maxScalingString = Application.properties.getProperty("scaling-value", "NaN");
                    ...
                    if (maxScalingValue > 0) {
                        ...
                        logger.debug("Scaling image " + currentImage.getFile().getName() + " with factor " + scaler);
                        Application.log("Scaling image " + currentImage.getFile().getName());
                    ...
                    Paragraph imageNumberParagraph = new Paragraph(String.valueOf(i), fontDefaultBold);
                    imageNumberParagraph.setAlignment(Element.ALIGN_CENTER);

                    logger.debug("Adding image " + currentImage.getFile().getName() + " to document");
                    Application.log("Adding image " + currentImage.getFile().getName() + " to document");

                    // Create upper page content
                    if ((i % 2) == 0) {

                        // Create first header line with company name
                        Paragraph headerFirstLine =
                                new Paragraph(Application.properties.getProperty("company", ""), fontDefaultBold);
                        ...
                        // suffix
                        Paragraph headerSecondLine = new Paragraph(headerText + headerSuffix, fontDefault);
                        ...
                        HeaderFooter header = new RtfHeaderFooter(headerElements);
                        ...
                        // Add image number
                        imageParagraph.add(new Phrase(GeneratorService.getFormattedImageNumber(i, images.size())));
                ...
                logger.error("Error while generating file: " + toFile.getAbsolutePath(), ex);
                Application.log("Error while generating file: " + toFile.getAbsolutePath());
            } finally {
                document.close();

                logger.debug("Document finished: " + toFile.getAbsolutePath());
                Application.log("Document finished: " + toFile.getAbsolutePath());
            }
    ...
    public static String getFormattedImageNumber(int imageNumber, int imageCount) {
        ...
        if (imageCount < 10) {
            ...
            if ((imageNumber + 1) < 10) {
                return ("00" + String.valueOf(imageNumber + 1));
            } else if((imageNumber + 1) < 100) {
                return ("0" + String.valueOf(imageNumber + 1));
            } else {
                return (String.valueOf(imageNumber + 1));
            }
